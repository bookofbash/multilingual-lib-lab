...
| phenomenon | n | tfidf | fasttext | glotlid | xlmr |
| --- | ---: | ---: | ---: | ---: | ---: |
| control | 10 | 1.000 | 1.000 | 1.000 | 0.900 |
| code_switching | 17 | 0.706 | 1.000 | 0.941 | 1.000 |
| related_language | 16 | 0.625 | 0.750 | 1.000 | 0.312 |
| very_short | 12 | 0.333 | 0.667 | 0.417 | 0.333 |
| romanization | 11 | 0.000 | 0.000 | 0.091 | 0.182 |
| low_resource | 13 | 0.077 | 0.692 | 1.000 | 0.077 |

GlotLID leads all-items exact because it can name the whole set (94 / 94).
fastText is close (92 / 94). XLM-R's all-items number looks like the sklearn
baseline until you read **covered exact** (0.769 on 65 items): the 20-class
head is decent on languages it has. The Korean control miss (`ko` → `ja`)
is coverage — Korean is not in that head — not a broken install.
Indonesian, Malay, and the mainland Scandinavian languages are also
missing, which is why `related_language` looks worse than `tfidf`.

Coverage is not the same as being right on short text. GlotLID's 2000-way
head is perfect on `low_resource` and `related_language`, then worse than
176-class fastText on `very_short`: `no` → Bribri, `ok` → Luo, `oui` →
`zxx`. Romanization still barely moves. Codeswitch hit is 0.95–1.0 once
the model is not a tiny n-gram SVM; GlotLID's one miss is
`今日のstandup長すぎた。` snapping to Chinese. The remaining misses are
still linguistic: isolated `ja`, romanized `arigatou gozaimasu`.
...
1. Fine-tune XLM-R on a larger public LID set and keep this challenge set held out.
2. Add a proper codeswitch head, or evaluate top-2 predictions against `+` labels.
3. Optional: score the same adapters on CommonLID so the stress set and the web benchmark sit side by side.
